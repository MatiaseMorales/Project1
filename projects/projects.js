...
if (uuidSeleccionado) {
  proyectoPrincipal = data.find(project => project.uuid === uuidSeleccionado);
} 

// Si no se encontró uno válido, usar el primero
...
// Excluir el proyecto principal de los otros
const otrosProyectos = data.filter(project => project.uuid !== uuidSeleccionado);
...
function mostrarOtrosProyectos(proyectos) {
  ...
  proyectos.forEach(proyecto => {
    ...
    div.querySelector('a').addEventListener('click', (event) => {
      ...
      // Guardar el nuevo UUID en localStorage
      ...
      const proyectosRestantes = proyectosData.filter(p => p.uuid !== nuevoUuid);
